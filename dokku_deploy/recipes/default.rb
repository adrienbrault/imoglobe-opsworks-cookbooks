...
node[:deploy].each do |application, deploy|
	...
	if deploy[:domains]
		...
		execute "git push #{deploy[:user]}@localhost:#{deploy[:domains].first} #{deploy[:scm][:revision]}"  do
			user deploy[:user]
			group deploy[:group]
			cwd deploy[:current_path]
			command "git push #{deploy[:user]}@localhost:#{deploy[:domains].first} #{deploy[:scm][:revision]}"
		end
	end
end
